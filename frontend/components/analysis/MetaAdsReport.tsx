...
export function MetaAdsReport({ report }: { report: MetaAdsReportData | null }) {
  ...
  return (
    <section className="mt-8 rounded-3xl border border-[#c9ddd4] bg-[#123d42] p-6 text-[#eef8ef] shadow-xl shadow-[#123d42]/10 sm:p-8">
      <p className="text-xs font-bold uppercase tracking-[0.18em] text-[#f5b7a5]">Informe inteligente</p>
      ...
      {report.recommendations.length > 0 && <div className="mt-7 border-t border-white/15 pt-6"><h3 className="text-sm font-semibold uppercase tracking-[0.12em] text-[#f5b7a5]">Próximos pasos</h3><ul className="mt-3 space-y-2 text-sm leading-6 text-[#d8eee4]">{report.recommendations.map((recommendation) => <li key={recommendation}>→ {recommendation}</li>)}</ul></div>}
    </section>
  );
}
