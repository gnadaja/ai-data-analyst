  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-[var(--background)] px-6 py-12 text-[var(--foreground)]">
      <div className="w-full max-w-md rounded-3xl border border-[var(--border)] bg-[var(--surface)] p-8 shadow-xl shadow-black/10">
        ...
        <h1 className="mt-12 text-3xl font-semibold tracking-[-0.03em] text-[var(--foreground)]">{dictionary.registerTitle}</h1>
        <p className="mt-3 text-[var(--text-soft)]">{dictionary.registerDescription}</p>
        <form className="mt-8 space-y-5" onSubmit={handleSubmit}>
          ...
          {message && <p role="status" className="rounded-xl bg-emerald-500/10 px-4 py-3 text-sm text-[var(--success)]">{message}</p>}
          <button disabled={loading} className="w-full rounded-full bg-[var(--primary)] px-5 py-3.5 font-semibold text-white transition hover:bg-[var(--primary-strong)] disabled:cursor-wait disabled:opacity-60">{loading ? dictionary.registerLoading : dictionary.registerSubmit}</button>
        </form>
        <p className="mt-7 text-center text-sm text-[var(--text-soft)]">{dictionary.hasAccount} <Link className="font-semibold text-[var(--primary-strong)]" href="/login">{dictionary.loginLink}</Link></p>
      </div>
    </main>
  );
}
